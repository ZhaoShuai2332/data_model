...
This script computes permutation importance for the trained SVM model
and saves the results to the outputs directory.

Usage:
    python -m feature_analysis.svm_importance
    
    Or from project root:
    python feature_analysis/svm_importance.py
"""
...
def main():
    """Main function to compute feature importance for SVM."""
    
    print("=" * 60)
    print("SVM - Feature Importance Analysis")
    print("=" * 60)
    
    # Paths
    model_dir = os.path.join(project_root, 'svm_model')
    ...
    if not os.path.exists(preprocessor_path):
        ...
        print("Please run the SVM model training first.")
        return
    ...
    # Load trained model
    model_path = os.path.join(output_dir, 'svm_model_final.pkl')
    ...
    # SVM uses sampled training data, so use smaller validation set for speed
    MAX_SAMPLES = 5000
    if len(X_val) > MAX_SAMPLES:
        print(f"Sampling validation set: {len(X_val)} -> {MAX_SAMPLES} for speed")
