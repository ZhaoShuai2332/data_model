...
# Logger class: output to terminal and file simultaneously
class TeeLogger:
...
logger = TeeLogger('mlp_model\\outputs\\res_print.txt')
sys.stdout = logger
...
def main():
    ...
    print("=" * 60)
    print("High Risk Customer Prediction - MLP Deep Learning Model")
    print("=" * 60)
    
    # Ensure output directory exists
    os.makedirs('mlp_model\\outputs', exist_ok=True)
    ...
    plt.figure(figsize=(8, 6))
    plt.plot(recall_curve, precision_curve, lw=2, label=f'MLP (AUC = {pr_auc:.4f})')
    plt.xlabel('Recall')
    plt.ylabel('Precision')
    plt.title('Precision-Recall Curve')
    plt.legend(loc="lower left")
    plt.grid(True, alpha=0.3)
    plt.savefig('mlp_model\\outputs\\pr_curve.png', dpi=150, bbox_inches='tight')
    plt.close()
    print("PR curve saved to: mlp_model\\outputs\\pr_curve.png")
    ...
    plt.legend()
    plt.grid(True, alpha=0.3)
    plt.savefig('mlp_model\\outputs\\f1_threshold.png', dpi=150, bbox_inches='tight')
    plt.close()
    print("F1 threshold plot saved to: mlp_model\\outputs\\f1_threshold.png")
    ...
    plt.savefig('mlp_model\\outputs\\confusion_matrix.png', dpi=150, bbox_inches='tight')
    plt.close()
    print("Confusion matrix saved to: mlp_model\\outputs\\confusion_matrix.png")
    ...
    print(f"Prediction completed, samples: {len(test_probs)}")
    ...
    print(f"File format: {len(test_probs)} rows x 1 column")
    ...
    }, 'mlp_model\\outputs\\mlp_model_final.pth')
    
    print("Model saved to: mlp_model\\outputs\\mlp_model_final.pth")
