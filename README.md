...
本项目针对高风险客户识别任务，通过对比实验探究了不同模型架构在表格数据上的性能表现。核心包含以下四大算法模块：

1.  **Transformer (Self-Attention)**: 采用多头自注意力机制捕捉特征间的全局依赖关系，参考 Vaswani et al. (2017) 与 Gorishniy et al. (2021) 架构。
2.  **Multilayer Perceptron (MLP)**: 经典的深度前馈神经网络，作为深度学习的基准模型。
3.  **Support Vector Machine (SVM)**: 基于核技巧 (Kernel Trick) 的分类器，适用于小样本高维空间。
4.  **Logistic Regression**: 广义线性模型，作为二分类任务的 Baseline。
...
- `outputs/`: **输出目录**，包含训练结果、评估图表、特征重要性分析 (`feature_importance.csv/png`) 等。
...
基于各模型在验证集 (Validation Set) 和训练集 (Training Set) 上的表现，统计如下关键指标。**Transformer** 模型在验证集 AUC 与 F1-Score 上均取得了最优结果。每个模型目录下的 `outputs/feature_importance.csv` 与 `outputs/feature_importance.png` 提供了详细的特征重要性分析。
...
4.  **特征重要性分析**: 所有模型均输出特征重要性排序，便于业务理解与特征工程迭代。
...
> *   深度学习模型 (Transformer, MLP) 在训练集和验证集上的 AUC 非常接近，说明泛化能力良好。
> *   SVM 在训练集上表现出较高的 Metrics (AUC 0.9204)，但在验证集上下降至 0.8829，存在一定的过拟合现象。
> *   **特征重要性**: 各模型的 `outputs/` 目录包含 `feature_importance.csv` 和 `feature_importance.png`，详细展示特征对预测的贡献度。
