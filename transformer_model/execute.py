# -*- coding: utf-8 -*-
"""
Transformer Execution Entry Point
"""
...
logger = TeeLogger('transformer_model\\outputs\\res_print.txt')
sys.stdout = logger
...
def main():
    ...
    print("=" * 60)
    print("High Risk Customer Prediction - Transformer Model")
    print("Using Self-Attention Mechanism for Tabular Data")
    print("=" * 60)
    
    # Ensure output directory exists
    os.makedirs('transformer_model\\outputs', exist_ok=True)
    ...
    print(f"Parameter count: {sum(p.numel() for p in predictor.model.parameters()):,}")
    ...
    # 5. Training set evaluation (for consistency with Logistic/SVM models)
    ...
    plt.savefig('transformer_model\\outputs\\roc_curve.png', dpi=150, bbox_inches='tight')
    plt.close()
    ...
    plt.figure(figsize=(8, 6))
    plt.plot(recall_curve, precision_curve, lw=2, label=f'Transformer (AUC = {pr_auc:.4f})')
    plt.xlabel('Recall')
    plt.ylabel('Precision')
    plt.title('Precision-Recall Curve')
    plt.legend(loc="lower left")
    plt.grid(True, alpha=0.3)
    plt.savefig('transformer_model\\outputs\\pr_curve.png', dpi=150, bbox_inches='tight')
    plt.close()
    print("PR curve saved: transformer_model\\outputs\\pr_curve.png")
    ...
    plt.legend()
    plt.grid(True, alpha=0.3)
    plt.savefig('transformer_model\\outputs\\f1_threshold.png', dpi=150, bbox_inches='tight')
    plt.close()
    print("F1 threshold plot saved: transformer_model\\outputs\\f1_threshold.png")
    ...
    plt.savefig('transformer_model\\outputs\\confusion_matrix.png', dpi=150, bbox_inches='tight')
    plt.close()
    print("Confusion matrix saved: transformer_model\\outputs\\confusion_matrix.png")
    ...
    result_df.to_csv('transformer_model\\outputs\\result.csv', 
                     encoding='utf-8', 
                     index=False, 
                     header=False)
    ...
    torch.save({
        ...
        'best_auc': best_auc,
    }, 'transformer_model\\outputs\\transformer_model_final.pth')
    
    print("Model saved: transformer_model\\outputs\\transformer_model_final.pth")
